...
orange_img = cv2.imread(
    "C:\LeeDongUk\Python\opencv\orenge.png"
...
predictor = dlib.shape_predictor(
    "C:\LeeDongUk\Python\opencv\shape_predictor_68_face_landmarks.dat"
)  # ** 68개 점의 랜드 마크를 탐지 **

cap = cv2.VideoCapture(cv2.CAP_DSHOW + 0)  # 0이면 본인의 웹캠이 켜지고 영상 파일 이름을 넣으면 영상 파일이 켜진다.
...
while cap.isOpened():
    ...
    if len(faces) > 0:  # 얼굴이 1개 이상이면, 즉 프레임이 여러개이므로 detect된 얼굴도 여러 개이겠지
        ...
        left_eye_img = img[
            le_y1 - le_margin : le_y2 + le_margin, le_x1 - le_margin : le_x2 + le_margin
        ].copy()
        right_eye_img = img[
            re_y1 - re_margin : re_y2 + re_margin, re_x1 - re_margin : re_x2 + re_margin
        ].copy()
        ...
        # 불투명하게 합성 (alpha 값을 조절하여 조절 가능)
        alpha = 2.3  # 0.0부터 1.0까지의 값을 사용하여 조절
        cv2.addWeighted(result, alpha, orange_img, 1 - alpha, 0, result)
